...
class UPMDataset(data.Dataset):
    def __init__(self, opt):
        ...
        for class_i, root in enumerate(self.dataroot):
            path = image_util.get_image_paths(root)
    ...
    def __getitem__(self, index):
        ...
        img = image_util.imread_uint(path, self.n_channels)
        img_c = img.copy()
        ...
        if self.opt['phase'] == 'train':
            ...
            patch = image_util.augment_img(patch, mode=mode)
            patch_c = image_util.augment_img(patch_c, mode=mode)

            # ---------------------------------
            # HWC to CHW, numpy(uint) to tensor
            # ---------------------------------
            img = image_util.uint2tensor3(patch)
            img_c = image_util.uint2tensor3(patch_c)
